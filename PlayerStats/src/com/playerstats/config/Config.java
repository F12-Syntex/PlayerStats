...
public abstract class Config {
	...
	public Config(String name, double version) {
		...
		this.base = PlayerStats.getInstance();
	...
	public File backup() {
		...
		PlayerStats.Log("Backing up...");
		
		if(backups.mkdirs() || backups.exists()) {

			PlayerStats.Log("created");
			...
			if(!this.getConfig().exists()) return null;
				...
				if(success) {
					PlayerStats.Log("&a" + this.name + ".yml has been backed up!");	
				}else {
					PlayerStats.Log("&cCouldnt backup!");
				}
			
				return tempConfig;
			
		}else if(!backups.exists()) {
			PlayerStats.Log("&cCouldnt create backup folder!");
		}
